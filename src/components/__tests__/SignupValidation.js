...
const signUpCombo = (firstname, lastname, email, password, passwordRetype, agree) => {
    ...
    fireEvent.change(screen.getByRole("lastname"), {target: {value: lastname}})
    ...
    fireEvent.change(screen.getByRole("passwordRetype"), {target: {value: passwordRetype}})
    if (agree) {
...
describe('Sign up validation', () => {
    test('Valid Case', () => {
        signUpCombo("John", "Doe", "johndoe@gmail.com", "Password123",
            "Password123", true)
        expect(screen.queryAllByText(errorRE)).toHaveLength(0)
        expect(screen.getByText("You must agree to our agreement")).not.toBeVisible()
    });
    test('Empty Firstname', () => {
        signUpCombo("", "Doe", "johndoe@gmail.com", "Password123",
            "Password123", true)
        expect(screen.queryAllByText(errorRE)).toHaveLength(1)
        expect(screen.getByText("You must agree to our agreement")).not.toBeVisible()
    });
    test('Empty Lastname', () => {
        signUpCombo("John", "", "johndoe@gmail.com", "Password123",
            "Password123", true)
        expect(screen.queryAllByText(errorRE)).toHaveLength(1)
        expect(screen.getByText("You must agree to our agreement")).not.toBeVisible()
    });
    test('Invalid Email', () => {
        signUpCombo("John", "Doe", "johndoe@", "Password123",
            "Password123", true)
        expect(screen.queryAllByText(errorRE)).toHaveLength(1)
        expect(screen.getByText("You must agree to our agreement")).not.toBeVisible()
    });
    test('Invalid password', () => {
        signUpCombo("John", "Doe", "johndoe@gmail.com", "password",
            "password", true)
        expect(screen.queryAllByText(errorRE)).toHaveLength(1)
        expect(screen.getByText("You must agree to our agreement")).not.toBeVisible()
    });
